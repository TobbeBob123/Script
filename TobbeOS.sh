...
yay -S htop kitty network-manager-applet lxsession dmenu trayer yad git lightdm lightdm-gtk-greeter zip feh scrot dunst pavucontrol nm-connection-editor vim libreoffice pulseaudio picom pcmanfm && \
                 rm -rf ~/.xmonad && \
                 rm -rf ~/.config/kitty && \
                 rm -rf ~/.config/xmobar && \
                 rm -rf ~/.config/dunst && \
                 ...
                 sudo mv /etc/lightdm/display-manager.service /etc/systemd/system/display-manager.service && \
